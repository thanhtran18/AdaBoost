...
for iter in range(niter):
    ...
    ##################### Fill in #############################
    ###########################################################
    err = (np.dot(weights, (1 - correct)) / sum(weights))
    if err > 0.5:
        break
    # alpha = 0
    alpha = (np.log((1 - err) / (err + 0.001))).reshape(1, 1)
    # test= (1-correct).T
    weights = np.multiply(weights, np.exp(np.dot(alpha, ((1-correct).reshape(500, 1)).T)))
